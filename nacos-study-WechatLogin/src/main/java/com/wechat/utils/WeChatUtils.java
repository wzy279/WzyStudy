...
import com.wechat.entity.MyX509TrustManager;
...
public class WeChatUtils {
	// 微信公众号的appId以及secret
	private static String appId = "wxf9f88c032121d358";
	private static String secret = "45dc6e3775a8a045eecfa1512c981cc9";
	...
	public static WeChatAccessToken getAccessToken(String code) {
		String url = WeChatUtils.getAccessTokenUrl.replace("APPID", WeChatUtils.appId).replace("SECRET", secret)
				.replace("CODE", code);
		/*JSONObject jsonObj=JSONObject.from(httpRequest(url, "POST", null));*/
		return (WeChatAccessToken) JSONObject.parseObject(httpRequest(url, "POST", null),WeChatAccessToken.class);
	}
	...
	public static WechatUserinfo getWXUserInfoUrl(String openId, String accessToken) {
		...
		System.out.println(httpRequest(url, "GET", null));
		return (WechatUserinfo) JSONObject.parseObject(httpRequest(url, "GET", null),WechatUserinfo.class);
	}
